...
test('ships versioned immutable assets and required static-host response policies', () => {
  ...
  expect(index).toContain('hero-card-640.webp?v=1.0.3');
  expect(index).toContain('apple-touch-icon');
  expect(manifest).toContain('"start_url": "/?source=installed&v=1.0.3"');
  expect(worker).toContain("next-step-cards-shell-v1.0.3");
